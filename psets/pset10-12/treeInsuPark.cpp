...
bool containsBT(tree node, int key) {
    ...
    if (containsBT(node->left, key))
        return true;
    if (containsBT(node->right, key))
        return true;
    //cout << "your code here \n";
	return false;
}
...
tree findBT(tree root, int key) {
    ...
    tree node = findBT(root->left, key);
    if (node != nullptr) {
        return node;
    }
    ...
    //return findBT(root->right, key);도 가능
...
bool findPathBack(tree root, tree x, vector<int>& path) {
    ...
    if (findPathBack(root->left, x, path)) {
        path.push_back(root->key);
        return true;
    }
    
    if (findPathBack(root->right, x, path)) {
        path.push_back(root->key);
        return true;
    }
	...
	// node with two childeren: replace it with the successor or predecessor
        ...
        if (node->left == nullptr && node->right == nullptr) {
            ...
            if (height(node->right) > height(node->left)) {
                ...
                node->right = trim(node->right, minimum(node->right)->key);
            }
            else {
                node->key = maximum(node->left)->key;
                node->left = trim(node->left, maximum(node->left)->key);
            }
            //cout << "your code here\n";
        }
    }
...
tree LCA_BT(tree root, tree p, tree q) {  // recursive solution
    ...
    if (LCA_BT(root->left, p, q) != nullptr && LCA_BT(root->right, p, q) != nullptr)
        return root;
    
    tree node = LCA_BT(root->left, p, q);
    if (node != nullptr)
        return node;
    
    node = LCA_BT(root->right, p, q);
    if (node != nullptr)
        return node;
	//cout << "your code here\n";
...
bool isBST(tree x, int min, int max) {
    ...
    if (x->left != nullptr && maximum(x->left)->key > x->key)
        return false;
    
    if (x->right != nullptr && minimum(x->right)->key < x->key)
        return false;
...
void BTtoBSTinorder(tree node, vector<int>& v) {
    if (empty(node)) return;

    BTtoBSTinorder(node->left, v);
    node->key = v.back();
    v.pop_back();
    BTtoBSTinorder(node->right, v);
}
...
tree trimN(tree root, int N, bool AVLtree) {
    ...
    assert(key.size() == treeSize);
    
    srand((unsigned int)time(NULL));
    for (int i = 0; i < key.size(); i++) {
        int j  = i + rand_extended() % (key.size() - i);
        swap(key[i], key[j]);
    }
    
    if (N > treeSize) {
        for (int i = 0; i < treeSize; i++) {
            root = trim(root, key.back());
            key.pop_back();
        }
    }
    else {
...
tree trimAVL(tree node, int key) {
	...
	// node with two childeren: replace it with the successor or predecessor
        ...
        if (node->left == nullptr && node->right == nullptr) {
            ...
            if (height(node->right) > height(node->left)) {
                node->key = minimum(node->right)->key;
                node->right = trimAVL(node->right, minimum(node->right)->key);
            }
            else {
                node->key = maximum(node->left)->key;
                node->left = trimAVL(node->left, maximum(node->left)->key);
            }
            //cout << "your code here\n";
        }
	}
