...
void rehash(Hash*& ht) {
    ...
    for (int i = 0; i < old_size; i++) {
        if (oldHash->hashtable[i].size() > 0) {
            for (auto x : oldHash->hashtable[i]) {
                insert(ht, x);
            }
        }
    }
    
    delete[] oldHash->hashtable;
...
bool erase(Hash*& ht, string key) {
    ...
    int index = hashfunction(ht, key);
    for (auto i = ht->hashtable[index].begin(); i != ht->hashtable[index].end(); i++) {
        if (*i == key) {
            ht->hashtable[index].erase(i);
            ht->nelements--;
            return true;
        }
    }
...
list<string> find(Hash* ht, string key) {
    ...
    if (ht->hashtable[index].size() > 0) {
        ...
        for (int i = 0; i < tablesize(ht); i++) {
            if (ht->hashtable[i] == ht->hashtable[index]) {
