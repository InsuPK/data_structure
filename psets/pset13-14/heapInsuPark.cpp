...
void swim(heap p, int k) {
    ...
    int k_saved = p->nodes[k];
    
    while(p->nodes[k] > 1 && ::less(p, p->nodes[k/2], p->nodes[k])) {
        swap(p, p->nodes[k]/2, p->nodes[k]);
        p->nodes[k] = p->nodes[k]/2;
    }
    
    cout << "   N=" << k << " k=" << k_saved << " ";
    for (int i = 1; i <= k; i++) cout << p->nodes[i] << " ";
    cout << endl;
...
void sink(heap p, int k) {
    ...
    while (2 * p->nodes[k] <= k) {
        int j = 2 * p->nodes[k];
        if (j < k && ::less(p, p->nodes[j], p->nodes[j+1])) j++;
        if (!(::less(p, p->nodes[k], j))) break;
        swap(p, p->nodes[k], j);
        p->nodes[k] = j;
    }
...
void growN(heap p, int count, bool heapOrdered) {
    ...
    for (int i = 0; i <= count; i++) {
        ...
        #ifdef DEBUG
            cout << "growN: ";
            for (int i = 1; i <= p->N; i++) cout << p->nodes[i] << " ";
            cout << endl;
        #endif
    }
