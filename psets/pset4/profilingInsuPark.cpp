...
int main(int argc, char *argv[]) {
	...
	int ins = 0, mer = 1, qui = 2;

	cout << "\n\t" << sort_st[ins] << "(): sorted" << endl;
	for (int i = 0; i < N; i++) list[i] = i;   			// sorted data ready
	profiling(sort_fp[ins], list, N, ::less);  		

	cout << "\n\t" << sort_st[ins] << "(): randomized" << endl;
	randomize(list, N);
	profiling(sort_fp[ins], list, N, ::less);         	// randomized 

	cout << "\n\t" << sort_st[ins] << "(): reversed" << endl;
	int j = N - 1;
	for (int i = 0; i < N; i++) list[i] = j--;			// reversed sequence
	profiling(sort_fp[ins], list, N, ::less);


	cout << "\n\t" << sort_st[mer] << "(): sorted" << endl;
	for (int i = 0; i < N; i++) list[i] = i;   			// sorted data ready
	profiling(sort_fp[mer], list, N, ::less);  		

	cout << "\n\t" << sort_st[mer] << "(): randomized" << endl;
	randomize(list, N);
	profiling(sort_fp[mer], list, N, ::less);         		// randomized

	cout << "\n\t" << sort_st[mer] << "(): reversed" << endl;
	j = N - 1;
	for (int i = 0; i < N; i++) list[i] = j--;			// reversed sequence
	profiling(sort_fp[mer], list, N, ::less);   	


	cout << "\n\t" << sort_st[qui] << ": sorted" << endl;
	for (int i = 0; i < N; i++) list[i] = i;   			// sorted data ready
	profiling(sort_fp[qui], list, N, ::less);              

	cout << "\n\t" << sort_st[qui] << "(): randomized" << endl;
	randomize(list, N);
	profiling(sort_fp[qui], list, N, ::less);              // randomized 

	cout << "\n\t" << sort_st[qui] << "(): reversed" << endl;
	j = N - 1;
	for (int i = 0; i < N; i++) list[i] = j--;			// reversed sequence
	profiling(sort_fp[qui], list, N, ::less);   
