...
pNode half(pList p) { 
    ...
    while (rabbit != end(p) && rabbit->next->next != nullptr) {
...
void pop_all(pList p, int val) {
	...
	for (pNode c = begin(p); c != end(p); c = x) {
        x = c->next;
...
void push_backN(pList p, int N) {
    ...
    for (int i = 0; i < N; i++) {
        int num = static_cast<int>(rand_extended());
        push_back(p, num%range);
        if (i % 10000 == 0)
            cout << setw(7) << "\r\tpushing [" << i + psize << "]=" << num%range << "        ";
    }
...
void unique(pList p) {
    ...
    pNode temp;
    for (pNode x = begin(p); x != end(p); x = temp) {
        temp = x->next;
...
// for example:   pList: 9 7 5 3   x: 6  returns 5
